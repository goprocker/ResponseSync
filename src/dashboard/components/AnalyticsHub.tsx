  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-5 space-y-5 text-[#e0e0e6] font-sans">
      ...
      <div className="bg-[#050507] p-6 rounded-none border border-white/10 shadow-none flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <span className="px-2.5 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-[0.2em] bg-brand/15 text-[#e0e0e6] border border-brand/30 flex items-center gap-1.5 w-fit mb-2">
            <Activity className="w-3.5 h-3.5 text-[#e0e0e6]" />
        ...
        <div className="flex items-center gap-3">
          <div className="bg-[#050507] px-4 py-2 rounded border border-white/10 text-right font-mono">
            <span className="text-[10px] text-[#888] uppercase font-bold block">Avg Response Speedup</span>
            <span className="text-lg font-bold text-brand">42% Faster</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[#050507] p-4 rounded-none border border-white/10 space-y-2">
          <div className="flex items-center justify-between">
            <CloudRain className="w-4 h-4 text-[#e0e0e6]" />
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          </div>
          <h4 className="font-bold text-[#e0e0e6] text-xs uppercase tracking-wider font-sans">Weather Radar Feed</h4>
          <p className="text-xs text-[#aaa]">OpenWeatherMap / IMD Radar</p>
          <div className="text-[11px] font-mono text-[#e0e0e6] font-bold">85 mm/hr Convective Cell</div>
        </div>
        ...
        <div className="bg-[#050507] p-4 rounded-none border border-white/10 space-y-2">
          ...
          <h4 className="font-bold text-[#e0e0e6] text-xs uppercase tracking-wider font-sans">Satellite SAR Observation</h4>
        ...
        <div className="bg-[#050507] p-4 rounded-none border border-white/10 space-y-2">
          ...
          <h4 className="font-bold text-[#e0e0e6] text-xs uppercase tracking-wider font-sans">IoT Hydrodynamic Sensors</h4>
          <p className="text-xs text-[#aaa]">5 Ultrasonic Sluice Gauges</p>
          <div className="text-[11px] font-mono text-brand font-bold">100% Telemetry Online</div>
        </div>
        ...
        <div className="bg-[#050507] p-4 rounded-none border border-white/10 space-y-2">
          ...
          <h4 className="font-bold text-[#e0e0e6] text-xs uppercase tracking-wider font-sans">Traffic & Corridors</h4>
      ...
      <div className="bg-[#050507] rounded-none border border-white/10 p-5 shadow-none space-y-4">
        <h3 className="font-bold text-[#e0e0e6] text-sm uppercase tracking-wider flex items-center gap-2 font-sans">
          <Radio className="w-4 h-4 text-[#e0e0e6]" />
          IoT Hydrodynamic Sensor Telemetry (Velachery & Adyar)
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sensors.map((sensor) => (
            <div key={sensor.id} className="bg-[#050507] p-4 rounded border border-white/5 space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[10px] font-mono font-bold text-[#888] uppercase">{sensor.id}</span>
                  <h4 className="font-bold text-[#e0e0e6] text-sm mt-0.5 font-sans">{sensor.name}</h4>
                </div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider ${
                  sensor.status === 'critical' ? 'bg-brand/20 text-[#e0e0e6] border border-brand/30' : 'bg-emerald-500/20 text-brand border border-emerald-500/30'
                }`}>
                  {sensor.status}
                </span>
              </div>
              ...
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-xl font-bold text-[#e0e0e6] font-mono">
              ...
              <div className="space-y-1 font-mono">
                ...
                <div className="w-full bg-[#050507] h-2 rounded overflow-hidden">
                  <div
                    className={`h-full ${sensor.currentValue >= sensor.thresholdCritical ? 'bg-brand' : 'bg-amber-400'}`}
                    style={{ width: `${Math.min(100, (sensor.currentValue / (sensor.thresholdCritical * 1.2)) * 100)}%` }}
                  ></div>
                </div>
              </div>

              <div className="flex justify-between text-[10px] text-[#888] pt-1 border-t border-white/5 font-mono">
