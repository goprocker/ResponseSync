  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {hospitals.map((h) => {
          const occupancyPct = Math.round((h.occupiedCapacity / h.totalCapacity) * 100);
          return (
            <div key={h.id} className="bg-[#0e0e14] border border-white/10 p-5 space-y-4 shadow-sm relative">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`text-[9px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${
                  h.status === 'normal' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                  h.status === 'near_capacity' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-mono">
                  ...
                  <span className="text-white font-bold">{h.occupiedCapacity} / {h.totalCapacity} ({occupancyPct}%)</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5 text-xs font-mono">
                <div>
                  ...
                  <span className={`font-bold ${h.icuBedsAvailable === 0 ? 'text-red-500' : 'text-white'}`}>
