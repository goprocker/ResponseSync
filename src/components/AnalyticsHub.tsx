...
import { IoTSensorNode, ZoneRisk } from '../types';
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6 text-[#e0e0e6] font-sans">
      ...
      <div className="bg-[#0a0a0f] p-6 rounded-lg border border-[#ffffff15] shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-[#151520] px-4 py-2 rounded border border-[#ffffff15] text-right font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[#0d0d14] p-4 rounded-lg border border-[#ffffff15] space-y-2">
          <div className="flex items-center justify-between">
            <CloudRain className="w-4 h-4 text-[#ff4e00]" />
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          </div>
          <h4 className="font-bold text-white text-xs uppercase tracking-wider font-sans">Weather Radar Feed</h4>
          <p className="text-xs text-[#aaa]">OpenWeatherMap / IMD Radar</p>
          <div className="text-[11px] font-mono text-[#ff4e00] font-bold">85 mm/hr Convective Cell</div>
        </div>
        ...
        <div className="bg-[#0d0d14] p-4 rounded-lg border border-[#ffffff15] space-y-2">
          ...
          <h4 className="font-bold text-white text-xs uppercase tracking-wider font-sans">Satellite SAR Observation</h4>
        ...
        <div className="bg-[#0d0d14] p-4 rounded-lg border border-[#ffffff15] space-y-2">
          ...
          <h4 className="font-bold text-white text-xs uppercase tracking-wider font-sans">IoT Hydrodynamic Sensors</h4>
        ...
        <div className="bg-[#0d0d14] p-4 rounded-lg border border-[#ffffff15] space-y-2">
          ...
          <h4 className="font-bold text-white text-xs uppercase tracking-wider font-sans">Traffic & Corridors</h4>
      ...
      <div className="bg-[#0d0d14] rounded-lg border border-[#ffffff15] p-5 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sensors.map((sensor) => (
            <div key={sensor.id} className="bg-[#151520] p-4 rounded border border-[#ffffff10] space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[10px] font-mono font-bold text-[#888] uppercase">{sensor.id}</span>
                  <h4 className="font-bold text-white text-sm mt-0.5 font-sans">{sensor.name}</h4>
                </div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider ${
                  sensor.status === 'critical' ? 'bg-[#ff4e00]/20 text-[#ff4e00] border border-[#ff4e00]/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                }`}>
                  {sensor.status}
                </span>
              </div>
              ...
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-xl font-bold text-[#ff4e00] font-mono">
              ...
              <div className="space-y-1 font-mono">
                ...
                <div className="w-full bg-[#0a0a0f] h-2 rounded overflow-hidden">
                  <div
                    className={`h-full ${sensor.currentValue >= sensor.thresholdCritical ? 'bg-[#ff4e00]' : 'bg-amber-400'}`}
                    style={{ width: `${Math.min(100, (sensor.currentValue / (sensor.thresholdCritical * 1.2)) * 100)}%` }}
                  ></div>
                </div>
              </div>

              <div className="flex justify-between text-[10px] text-[#888] pt-1 border-t border-[#ffffff10] font-mono">
